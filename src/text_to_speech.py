...
Uses calm, measured ENGLISH voices suited to a reflective Islamic channel, and
extracts word-level timing (WordBoundary events) so the renderer can draw
captions synced to the voice.
"""
import asyncio
import subprocess
import tempfile
...
# Calm, clear English neural voices (natural, measured — not hype)
VOICES = [
    "en-US-ChristopherNeural",  # male, warm and calm
    "en-GB-RyanNeural",         # male, British, measured
...
def generate_audio(text: str, output_path: str, voice: str = VOICES[0], rate: str = "+0%"):
    """
...
def build_narration(segments: list[tuple[str, str]], output_path: str,
                    voice: str = VOICES[0], rate: str = "+0%") -> tuple[list[dict], list[dict]]:
    """
    Synthesizes multiple labelled text segments into one audio file.

    segments: list of (label, text) — e.g. [("intro", "..."), ("point_1", "..."), ...]
...
if __name__ == "__main__":
    ...
    print(f"{len(words)} words with timing captured")
    for w in words[:5]:
